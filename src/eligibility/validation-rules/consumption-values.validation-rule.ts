...
import { ConsumptionHistoryDto, CustomerEligibilityRequestDto } from '../dto/customer-eligibility-calc-request.dto';
...
export class ConsumptionValuesValidationRule extends BaseValidationRule {
  ...
  private readonly MIN_AVG_IN_kWh_FOR_CONNECTION_TYPE_MAPPING: Map<ConnectionTypesEnum, number> = new Map([
  ...
  async validate(
    ...
    const minAvgValue = this.MIN_AVG_IN_kWh_FOR_CONNECTION_TYPE_MAPPING.get(customerInfo.tipoDeConexao)
    if (!minAvgValue) {
      throw new Error('value not expected')
    }

    const isValidConsumption = this.isValidConsumption(customerInfo.historicoDeConsumo, minAvgValue)
  ...
  private calculateAverageConsumption(consumptionHistory: ConsumptionHistoryDto) {
    const sum = consumptionHistory.items.reduce((prev, curr) => prev + curr);
    const avg = (sum / consumptionHistory.items.length) || 0;
    return avg
  }

  private isValidConsumption(consumptionHistory: ConsumptionHistoryDto, minAverageConsumptionValue: number) {
